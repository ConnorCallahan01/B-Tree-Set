...
namespace main_savitch_11
{
    ...
    public:
        ...
        set( );                                           //CHECK
        set(const set& source);                           //CHECK
        ~set( ) { clear( ); }                             //CHECK
        // MODIFICATION MEMBER FUNCTIONS
        void operator =(const set& source);               //CHECK
        void clear( );                                    //CHECK
        bool insert(const Item& entry);                   //CHECK
        std::size_t erase(const Item& target);
        // CONSTANT MEMBER FUNCTIONS
        std::size_t count(const Item& target) const;      //CHECK
        bool empty( ) const { return (data_count == 0); } //CHECK
    ...
    private:
        ...
        bool loose_insert(const Item& entry);             //CHECK
        bool loose_erase(const Item& target);             //CHECK
        void remove_biggest(Item& removed_entry);
        void fix_excess(std::size_t i);                   //CHECK
        void fix_shortage(std::size_t i);                 //CHECK
        // NOTE: The implementor may want to have additional helper functions
        //
        //additional helper functions
        set* b_tree_copy(const set* root_ptr);            //CHECK
        void b_tree_clear(set*& root_ptr);                //CHECK
        std::size_t get_index(const Item& entry);         //CHECK
        bool within_subset(int index) const{ return (index >= 0) && (index < child_count); } //CHECK
        //
        //
      };
}
